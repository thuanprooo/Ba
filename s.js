...
const browsers = ["chrome", "safari", "brave", "firefox", "mobile", "opera","coccoc","kiwibrowser"];
...
let headers = {
  ...
  ":scheme": "https",
  "accept": accept_header[Math.floor(Math.random() * accept_header.length)] || "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,*/*;q=0.8",
  "accept-encoding": encoding_header[Math.floor(Math.random() * encoding_header.length)] || "gzip, deflate, br, zstd",
  ...
  "cache-control": randomElement(["no-cache", "max-age=0", "no-store", "must-revalidate"]),
  "sec-ch-ua": secChUa || `"Not/A)Brand";v="99", "Chromium";v="${Math.floor(Math.random() * 20) + 100}", "Google Chrome";v="${Math.floor(Math.random() * 20) + 100}"`,
  "sec-ch-ua-mobile": randomElement(["?0", "?1"]),
  "sec-ch-ua-platform": randomElement(["Android", "iOS", "Windows", "macOS", "Linux", "Chrome OS", "iPadOS"]),
  "sec-ch-ua-platform-version": randomElement([
    `"${Math.floor(Math.random() * 15)}.0.0"`,
    `"${Math.floor(Math.random() * 11) + 10}.${Math.floor(Math.random() * 10)}.0"`,
    `"${Math.floor(Math.random() * 24)}.${Math.floor(Math.random() * 10)}.${Math.floor(Math.random() * 10)}"`
  ]),
  "sec-fetch-dest": fetch_dest[Math.floor(Math.random() * fetch_dest.length)] || "document",
  "sec-fetch-mode": fetch_mode[Math.floor(Math.random() * fetch_mode.length)] || "navigate",
  "sec-fetch-site": fetch_site[Math.floor(Math.random() * fetch_site.length)] || "same-origin",
  "sec-fetch-user": "?1",
  "user-agent": u[Math.floor(Math.random() * u.length)] || `Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/${Math.floor(Math.random() * 20) + 100}.0.0.0 Safari/537.36`,
  "x-requested-with": randomElement(["XMLHttpRequest", "Fetch", ""]),
  "referer": `https://${parsedTarget.host}${parsedTarget.path.split('?')[0] || '/'}`,
  "upgrade-insecure-requests": "1",
  "priority": randomElement(["u=0, i", "u=1", "u=2", "u=3"]),
  "sec-ch-ua-full-version-list": `"Not/A)Brand";v="99.0.0.0", "Chromium";v="${Math.floor(Math.random() * 20) + 100}.0.${Math.floor(Math.random() * 5000)}.${Math.floor(Math.random() * 100)}", "Google Chrome";v="${Math.floor(Math.random() * 20) + 100}.0.${Math.floor(Math.random() * 5000)}.${Math.floor(Math.random() * 100)}"`,
  "dnt": randomElement(["0", "1"]),
  "te": randomElement(["trailers", ""]),
  "sec-purpose": randomElement(["prefetch", "prerender", ""]),
  "x-forwarded-for": `${Math.floor(Math.random() * 255)}.${Math.floor(Math.random() * 255)}.${Math.floor(Math.random() * 255)}.${Math.floor(Math.random() * 255)}`,
  "via": randomElement(["1.1 google", "1.0 proxy", "2.0 cloudflare", ""]),
  "if-none-match": randomElement([`"etag-${Math.random().toString(36).substring(2, 15)}"`, ""]),
  "cookie": randomElement([`session_id=${Math.random().toString(36).substring(2, 15)}`, ""])
}
